...
已创建可导入 Android Studio 的原生 Java 项目。当前源码版本为 0.5.0，尚未打包；最近的 Debug APK 为 `releases/time-display-0.4.2-debug.apk`。用户反馈已打包版本均在设备上完成验收；0.5.0 尚未进行设备验收，步骤见[测试计划](docs/test-plan.md)。
...
0.5.0 更新：每次导入的字体各自复制到应用数据目录，在字体菜单中以字体名称显示，并可用菜单项右侧的“×”删除。新增默认折叠的字体颜色控制，提供对齐的透明度、颜色强度、颜色、饱和四条彩色滑条。仅更新源码，暂未打包 APK。
