...
已创建可导入 Android Studio 的原生 Java 项目。当前源码版本为 0.9.3，尚未打包；最新 APK 为 `releases/time-display-0.9.2-debug.apk`。最新 APK 构建和签名已核对；字体大小调节尚待设备验收，步骤见[测试计划](docs/test-plan.md)。
...
0.9.3 源码更新：右侧字体设置增加 50%–200% 大小滑条和重置按钮，默认 100%；时钟及日期等文字随之缩放，并按屏幕空间避免越界。本版仅修改源码，尚未打包 APK。
