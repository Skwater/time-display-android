...
已创建可导入 Android Studio 的原生 Java 项目。当前源码版本为 0.6.0，本次按要求不打包 APK。最新已打包版本仍为 0.5.0，位于 `releases/time-display-0.5.0-debug.apk`；其设备验收结果尚未记录，步骤见[测试计划](docs/test-plan.md)。
...
0.6.0 源码更新：背景布局提供“填充、适应、拉伸、平铺”；左侧新增独立的应用语言选项，默认跟随系统，可选中文或英文。应用名称改为“屏幕时钟”，图标为小圆角黑底、中央白色“T:me”。图片与动图支持平铺；视频选平铺时以适应方式显示。本次未打包 APK。
