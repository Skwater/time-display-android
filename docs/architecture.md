...
- 字体大小百分比存入 `ClockSettings.FONT_SIZE_PERCENT`，默认 100%，范围 50%–200%。`ClockFaceView` 在原有时间和详情字号上乘以比例，再按画面宽高压缩超出空间的文字；右侧滑条即时更新并提供仅重置大小的按钮。
...
- 默认背景由黑色根视图上方的一层纯色视图绘制，颜色按 HSV 的色相、饱和度、强度和透明度计算；没有媒体时及媒体留白处可见。滑条修改 `SharedPreferences` 后直接更新该层，不重新加载媒体。
...
- 每次应用进程启动时只调度一次后台孤立文件清理。清理任务读取所有列表中的图片 URI，计算仍应保留的缩略图文件名，并保护旧版图片副本及可能被单个背景使用的文件；只遍历 `files/playlist-thumbnails/` 和 `files/playlist-images/` 两个应用私有目录。图片导入、缩略图生成与清理共用锁，避免新建文件尚未写入数据库时被误删。清理失败不影响启动，下次进程启动重试；界面不显示提示。
