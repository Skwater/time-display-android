...
- `PlaylistStore` 用 JSON 保存有序条目；图片复制到 `files/playlist-images/`，视频保留持久化文档 URI。删除图片条目时删除对应副本。
- `PlaylistFolderImporter` 通过 `ACTION_OPEN_DOCUMENT_TREE` 的持久读取授权查询子文档，递归扫描文件夹，按 MIME 类型及常见扩展名识别图片和视频，按路径排序后加入列表。导入在后台线程执行；最多扫描 10000 个文档，列表上限仍为 100 项。图片复制后不依赖原文件夹；视频条目记录树授权，删除最后一条相关视频时释放授权。仅导入图片的文件夹在导入完成后释放授权。
