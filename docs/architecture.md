...
- `PlaylistStore` 用应用私有 SQLite 数据库存储多个命名列表及有序项目；首次建库时将旧版 JSON 单列表迁入默认列表。图片复制到 `files/playlist-images/`，视频保留持久化文档 URI。另存为复制条目记录并复用图片副本和视频授权；仅在所有列表都不再引用时删除图片副本或释放授权。列表无固定项目上限，管理页每页读取 50 项；播放时读取当前列表的项目顺序。
- `PlaylistFolderImporter` 通过 `ACTION_OPEN_DOCUMENT_TREE` 的持久读取授权查询子文档，递归扫描文件夹，按 MIME 类型及常见扩展名识别图片和视频。先在后台扫描计数；如果导入使当前列表首次达到或超过 100 项，则在导入前请求确认；随后在后台再次扫描并导入，不设固定扫描数量上限。图片复制后不依赖原文件夹；视频条目记录树授权，删除最后一条相关视频时释放授权。仅导入图片的文件夹在导入完成后释放授权。
